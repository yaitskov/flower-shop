    ...
    public function init()
    {
        if(!isset($this->name))
          throw new CHttpException(500,
                      'Input field name for "tiny_mce" have to be set!');
        ...
        if($this->type === 'tinymce')
            $this->path = Yii::app()->getAssetManager()
              ->publish(dirname(__FILE__).DIRECTORY_SEPARATOR."tiny_mce");
        else
            $this->path = Yii::app()->getAssetManager()
              ->publish(dirname(__FILE__).DIRECTORY_SEPARATOR."fckeditor");
